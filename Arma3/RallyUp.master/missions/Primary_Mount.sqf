...
if (!isServer) exitwith {};
	...
	// Find Tasking Location
		_Defend_Location = [_TaskingPosition,_minDistance,_maxDistance] call RallyUp_fnc_position_LocationsBuilding;
		if (_Defend_Location distance2d _TaskingPosition > _maxDistance) exitWith {diag_log ["RallyUpDiag : Mission outside of _maxDistance"];true};;	// Will cancel if the task to to far away		
