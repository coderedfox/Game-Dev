...
if (!isServer) exitwith {};
	...
	// Find Tasking Location	
		_Reward_Location = [_TaskingPosition,_minDistance,_maxDistance,RallyUp_LocationsPoints,5] call RallyUp_fnc_position_Locations;
			if (_Reward_Location distance2d _TaskingPosition > _maxDistance) exitWith {diag_log ["RallyUpDiag : Mission outside of _maxDistance"];true};	// Will cancel if the task to to far away	
	...
	// Diary
		...
		// Create Guards		
		[_Reward_Location,0,100,["Group","Static","Vehicles"],_waveamount*2,"PATROL"] spawn RallyUp_fnc_create_Enemy;	
		
		// Task Event
		waituntil {
			sleep 15;			
			_areaTrigger = [allplayers,(RallyUpBluFor select 0),_Reward_Location,_minDistance,75] call RallyUp_fnc_task_isTriggered;							
		...
		waituntil {
			sleep 120;			
			_areaTrigger = [allplayers,(RallyUpBluFor select 0),_Reward_Location,_minDistance,75] call RallyUp_fnc_task_isTriggered;
			_EnemyTrigger = [RallyUp_TotalEnemyUnits,(RallyUpOpFor select 0),_Reward_Location,_minDistance,10] call RallyUp_fnc_task_isTriggered;						
			if (_areaTrigger && !_EnemyTrigger && Alive _Reward) exitWith {_Task_Status = "Succeeded";true};
